...
export type TaskColumn = keyof TaskDto | "status" | "discussionName" | "workspace";
...
function useTaskColumns({
  ...
  const columnMap: Partial<Record<TaskColumn, ColumnDef<TaskRow>>> = {
    ...
    status: {
      ...
      sortingFn: (rowA, rowB) => (rowA.original.status?.id ?? 0) - (rowB.original.status?.id ?? 0),
      cell: ({ row: { original: { id, status, assignee } } }) => (
        <StatusCell status={status} taskId={id} assigneeId={assignee.id} onUpdate={onUpdateStatus} />
      ),
    },
    assigneeStatuses: {
      id: "assigneeStatuses",
      ...
      filterFn: multiSelectFilter,
      sortingFn: "text",
      cell: ({ row: { original: { assignee, otherAssignees } } }) => {
        const relatedDirectives = otherAssignees.map(s => ({ assignee: s.assignee, status: s.status }));
        return <ResponsibleCell responsible={assignee} relatedDirectives={relatedDirectives} />;
      },
    },
    deadlineType: {
    ...
    discussionName: {
      ...
      cell: ({
        row: {
          original: { source: { name, date, attachmentKey } },
        },
      }) => {

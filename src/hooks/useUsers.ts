  ...
  return useQuery({
    ...
    queryFn: async (): Promise<IUser[]> =>
      USE_MOCK_API
        ...
        : await apiRequest<IUser[]>({ method: 'GET', url: usersUrl }),
    ...options,
  });
}
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<IUser[]> =>
      USE_MOCK_API
        ...
        : await apiRequest<IUser[]>({ method: 'GET', url: `${usersUrl}?workspace=${urlName}` }),
    ...options,
  });
}
  ...
  return useMutation({
    mutationFn: async (data: ICreateUser) =>
      USE_MOCK_API
        ...
        : await apiRequest<IUser>({ method: 'POST', url: usersUrl, data }),
    ...options,
    onSuccess: (...args) => {
  ...
  return useMutation({
    mutationFn: async ({ userId, data }: UpdateUserParams) =>
      USE_MOCK_API
        ...
        : await apiRequest<IUser>({ method: 'PATCH', url: `${usersUrl}/${userId}`, data }),
    ...options,
    onSuccess: (...args) => {
      queryClient.invalidateQueries({ queryKey: userKeys.all });
      options?.onSuccess?.(...args);
  ...
  return useMutation({
    mutationFn: async ({ userId, urlName }: DeleteUserParams) =>
      USE_MOCK_API
        ...
        : await apiRequest<void>({ method: 'DELETE', url: `${usersUrl}/${userId}` }),
    ...options,
    onSuccess: (...args) => {
  ...
  return useMutation({
    mutationFn: async ({ userId, urlName }: AddUserToWorkspaceParams) =>
      USE_MOCK_API
        ...
        : await apiRequest<void>({ method: 'POST', url: `${usersUrl}/${userId}/workspace/${urlName}` }),
    ...options,
    onSuccess: (...args) => {
      const { urlName } = args[1];
      queryClient.invalidateQueries({ queryKey: userKeys.workspace(urlName) });

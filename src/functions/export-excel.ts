...
import { DEADLINE_LABELS, DeadlineType } from "src/components/shared/DeadlineTag";
...
function lighten(hex: string, alpha: number): string {
  ...
  return "FF" + blended.map(v => v.toString(16).padStart(2, "0")).join("").toUpperCase();
}
...
const COLUMN_DEFS: Record<string, ExportColumn<TaskRow>> = {
  ...
  deadlineType: {
    ...
    accessor: (t) => {
      ...
      const value = typeStr && dateStr ? `${typeStr} | ${dateStr}` : typeStr || dateStr;
  ...
  tags: { header: "נושא", accessor: (t) => t.tags.map(({ name }) => name).join(", ") },
...
export async function exportTasksToExcel(tasks: TaskRow[], options: ExportOptions) {

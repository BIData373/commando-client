...
export function WorkspaceProvider({ children }: PropsWithChildren) {
  ...
  const { data, isLoading: isWorkspaceLoading, queryKey } = useListWorkspaces({ urlName })
  const workspace = data?.[0]
  const { data: workspaceStatuses, isLoading: isStatusesLoading } = useListWorkspaceStatuses(
    { workspaceId: workspace?.id! },
    { query: { enabled: !!workspace?.id } }
  )
  const statuses = Object.fromEntries((workspaceStatuses ?? []).map(s => [s.id, s]))
